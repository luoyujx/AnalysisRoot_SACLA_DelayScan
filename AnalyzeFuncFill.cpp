...
{	
	...
	//hi.fill(hiOff++,"FELintensityVsEnergy",ph.E(),intensity[1],"Energy [eV]","FEL intensity [arb. unit]",150,0,150,60,0,600,Form("%s/Energy",p.GetName()));
	//hi.fill(hiOff++,"DelayVsFELintensityVsEnergy",ph.E(),delay,intensity[1],"Energy [eV]","delay [ps]","FEL intensity [arb. unit]",150,0,150,delayBins,delayFrom,delayTo,60,0,600,Form("%s/Delay",p.GetName()));
	//hi.fill(hiOff++,"DelayVsFELintensityVsEnergy",ph.E(),delay,intensity[1],"Energy [eV]","delay [ps]","FEL intensity [arb. unit]",500,0,500,32,-5,11,30,0,600,Form("%s/Delay",p.GetName()));
		...
		for (size_t j = 0; j < p2.GetNbrOfParticleHits(); ++j)//i=j
		{
			for (size_t k = 0; k < p3.GetNbrOfParticleHits(); ++k)//i=j
			{
				...
				if ((TMath::Abs(pvecSumCII.X()) < pxSumWidth) && (TMath::Abs(pvecSumCII.Y()) < pySumWidth) && (TMath::Abs(pvecSumCII.Z()) < pzSumWidth))
				{
					...
					hi.fill(hiOffResult + 30, "KE_C", p1[i].E(), "KE [eV]", 300, 0, 300, Form("%s/KE", Hname.Data()));
					hi.fill(hiOffResult + 31, "KE_I1", p2[j].E(), "KE [eV]", 300, 0, 300, Form("%s/KE", Hname.Data()));
					hi.fill(hiOffResult + 32, "KE_I2", p3[k].E(), "KE [eV]", 300, 0, 300, Form("%s/KE", Hname.Data()));

					hi.fill(hiOffResult + 33, "Momentum_C", p1[i].P(), "Momentum [a.u.]", 1500, 0, 1500, Form("%s/Momentum", Hname.Data()));
