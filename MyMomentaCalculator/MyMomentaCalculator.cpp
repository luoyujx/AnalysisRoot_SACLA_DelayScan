...
double getMassIter(double tof_ns, const SpecRegions &sr)
{
	//when a negative time was given return immediatly because there will be no good solution//
	if (tof_ns < 0) return 1e15;

	double e = sr[0].EField_Vpcm() /10 ;// * MyUnitsConv::VPcm2mmPns();	//the acceleration in Region 1 in mm/ns^2

	//begin with a v when there would be only one Region//
	double m0  = e * 0.5 * tof_ns * tof_ns / sr[0].Length_mm();
	//std::cout<<"m0="<<m0 * MyUnitsConv::au2amu()<<std::endl;
	double fx0 = evalFunc(0,m0,1,sr);
	...
	while(TMath::Abs(fx0 - tof_ns) > 0.01)
	{
		...
		//a second point which is close to x0//
		double m1  = 0.9 * m0;
		double fx1 = evalFunc(0,m1,1,sr);

		//calculate the slope//
		double s = (fx0-fx1)/(m0-m1);

		//the next starting point is the point where the slopeline crosses the wanted fx0 value//
		m0  = m0 + 0.5*(tof_ns-fx0)/s;
		fx0 = evalFunc(0,m0,1,sr);
		//std::cout<<"m0="<<m0 * MyUnitsConv::au2amu()<<std::endl;
		//std::cout<<"fx0="<<fx0<<std::endl;

	}

	return m0 * MyUnitsConv::au2amu();
}
//#################################################################################
...
double MyMomentaCalculator::tof(double massQ, const MySpectrometer& sp)
{
	if (sp.GetSpectrometerRegions().size() == 1)	//if there is only one region
		return TMath::Sqrt(massQ/sp.GetSpectrometerRegions()[0].EField_Vpcm());
	else
	{
		std::cout<< "No Spectrometer Region has been set. Please set it, otherwise one cannot calculate the momentum"<<std::endl;
		return 0;
