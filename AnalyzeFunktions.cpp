...
void DefineParticlesAndRootFile(MyParticleContainer &particles, MyHistos &hi, const TString &whichParticles)
{
	...
	else if (whichParticles == "CH2I2") //20170323 SCSS+解析のために追加
	{
		//---CH2I2 for SCSS2016B
		AddCH2I2(particles);
	}
			...
			for (size_t j = 1; j < fParticles.GetNbrOfParticles(); ++j)
			{
				...
				for (size_t k = j; k < fParticles.GetNbrOfParticles(); ++k)
				{
					...
					if (kp.GetKindParticle() != 13) continue;
					{
						...
						fillMoleculeHistogramCH2I2_3body(ip, jp, kp, fIntensities, fHi, startIdx, molecule3, fDelays, delayBins, delayFrom, delayTo, fParticles.GetParticle(1).GetNbrOfParticleHits());//H2
						startIdx += 115;
					}
				}
			}
